...
- `/screenshots/{filename}` 路由通过同一端口提供服务，截图文件来自宿主机 `/var/lib/pi-dashboard/screenshots/`（由 compose 中 `/var/lib/pi-dashboard` 卷挂载持久化）

## 截图持久化

`pi_get_dashboard_screenshot` 调用流程：

1. `tools.py` 通过 `ipc_client.py` 请求 `pi_dashboard` 实时截图。
2. `screenshot_store.py` 将 base64 PNG 保存到 `/var/lib/pi-dashboard/screenshots/{timestamp}_{uuid}.png`。
3. 返回 `ImageContent`（标准 MCP） + `TextContent`（含文件名、大小、URL）。
4. `/screenshots/{filename}` 路由提供 HTTP 访问，文件名经白名单校验防止目录遍历。

`pi_dashboard_cleanup_screenshots` 支持按保存时长和保留数量两种维度清理。
